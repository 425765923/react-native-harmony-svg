...
void SvgPattern::ConcatMatrix(OH_Drawing_Canvas *canvas)
{
    ...
    Rect eRect = Rect(patternAttr_.x.ConvertToPx(), patternAttr_.y.ConvertToPx(), patternAttr_.width.ConvertToPx(), patternAttr_.height.ConvertToPx());
...
void SvgPattern::SaveDefinition(OH_Drawing_Canvas *canvas)
{
    ...
    if (patternAttr_.patternUnits) {
        LOG(INFO) << "xxxScale";
        OH_Drawing_CanvasScale(newCanvas, patternAttr_.width.ConvertToPx() / scale_, patternAttr_.height.ConvertToPx() / scale_);
    }
